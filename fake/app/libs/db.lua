...
function DB:exec(sql)
    ...
    ngx.log(ngx.INFO, "reused_times:", db:get_reused_times(), " sql:", sql)
    ...
    local ok, err2 = db:set_keepalive(conf.pool_config.max_idle_timeout, conf.pool_config.pool_size)
    if not ok then
        ngx.say("failed to set keepalive: ", err2)
    end
